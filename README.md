...
> A fully functional two-player terminal chess game, written in Python, with [Pydantic](https://docs.pydantic.dev/latest/) as the data layer and [Textual](https://textual.textualize.io/) and [Rich](rich.readthedocs.io/en/latest/) rendering the board.
...
Pydantic is the foundation. The board, each piece, every position, and each recorded move are all Pydantic models.

The move engine separates raw reachability from legality. One layer generates the squares each piece can physically reach; the other filters those candidates by simulating each move on a scratch board and keeping only the ones that don't leave the moving side's king in check.
...
cd dunder-chess
uv run __chess__ # or uv run dunder-chess
```
